...
#        down_url_html = http.get_tag_html(html,"a",{"class":"fast_download"})[0]
        """
        for i in down_url_html:
            if "normal_download" in i:
                down_url_html = i
                print i
        """
#        print down_url_html
        down_url = http.get_tag_attrs(html,"a",{"class": "fast_download"})[0]["bddlurl"]
#        down_url = http.get_tag_attrs(html,"a",{"class": "fast_download"})
#        print down_url
#        down_url = re.findall('href="(.*)"',down_url_html)[0]
        down_url = u"\n\n下载地址：" + down_url + "\n\n%s" % (u"原文：%s" % soft_url)

        return soft_info + soft_message1 + soft_message2 + down_url

    def android(self,soft):
...
#        soft_desc = http.get_tag_html(html,"div",{"class":"section-container introduction"})[0]
