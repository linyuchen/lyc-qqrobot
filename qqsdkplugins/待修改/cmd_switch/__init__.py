<<<<<<< HEAD:qqsdkplugins/待修改/cmd_switch/__init__.py
#coding=UTF8
...
class MyEvent(MsgEvent):
    ...
    def main(self,msg):
        ...
        if self.cmdClose.az(msg.msg):
            if member.isAdmin:
                param = self.cmdClose.get_param_list()
                for p in param:
                    result += u"%s 命令已经关闭\n" % (p)
            ...
            if member.isAdmin:
                param = self.cmdOpen.get_param_list()
                for p in param:
                    result += u"%s 命令已经开启\n" % (p)
        ...
        # 白名单
            ...
            if member.isAdmin:
                param = self.cmdAddWhite.get_param_list()
                for p in param:
                    result += u"%s 命令已加入白名单\n" % (p)
            ...
            if member.isAdmin:
                param = self.cmdDelWhite.get_param_list()
                for p in param:
...
>>>>>>> cd04e7609aa41a427ce4bf4b29e124de6a13fa90:qqsdkplugins/待修改/cmd_switch/__init__.py
